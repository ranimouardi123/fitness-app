      ...
      body: _bilans.isEmpty
          ...
          : SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                // Tableau comparatif tous bilans
                if (_bilans.length >= 1) ...[
                  _TableauComparatif(bilans: _bilans),
                  const SizedBox(height: 16),
                ],

                // Résultats totaux (si >= 2 bilans)
                if (_bilans.length >= 2) ...[
                  _ResultatsSemaines(bilans: _bilans),
                  const SizedBox(height: 16),
                ],

                // Objectifs cibles
                if (_bilans.isNotEmpty && _bilans.first['obj_poids'] != null) ...[
                  _ObjectifsCibles(bilan: _bilans.first),
                  const SizedBox(height: 16),
                ],
                ...
                if (_bilans.isNotEmpty && _bilans.first['synthese'] != null &&
                  ...
                  _SyntheseCoach(synthese: _bilans.first['synthese']),
                  const SizedBox(height: 16),
                ],

                // Boutons modifier/supprimer chaque bilan
                const Text('Bilans enregistrés',
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800,
                        color: AppTheme.grisTexte, letterSpacing: 1.5)),
...
class _TableauComparatif extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final displayed = bilans.take(4).toList(); // max 4 colonnes
    return Container(
        ...
        child: Table(
          ...
          children: [
            ...
            ..._indicateurs.map((ind) {
              ...
              final hasAny = vals.any((v) => v != null);
              if (!hasAny) return const TableRow(children: []);
              return TableRow(
                decoration: BoxDecoration(
                ...
                children: [
                  ...
                  ...vals.asMap().entries.map((e) {
                    ...
                    if (val != null && prev != null) {
                      ...
                      // Pour masse musculaire: augmentation = bon
...
Widget _ValCell(String val, Color? deltaColor, String deltaStr) => Padding(
  ...
  child: Column(mainAxisSize: MainAxisSize.min, children: [
    Text(val, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.noir),
        textAlign: TextAlign.center),
    if (deltaColor != null && deltaStr.isNotEmpty)
      Text(deltaStr, style: TextStyle(fontSize: 10, color: deltaColor, fontWeight: FontWeight.w600),
          textAlign: TextAlign.center),
  ]),
);

// ═══ RÉSULTATS EN X SEMAINES ═══
class _ResultatsSemaines extends StatelessWidget {
  final List<Map<String, dynamic>> bilans;
  const _ResultatsSemaines({required this.bilans});

  String _diff(Map<String, dynamic> a, Map<String, dynamic> b, String key,
      {bool reverse = false, String suffix = ''}) {
    final va = double.tryParse('${a[key]}');
    final vb = double.tryParse('${b[key]}');
    if (va == null || vb == null) return '—';
    final d = va - vb;
    final bon = reverse ? d > 0 : d < 0;
    return '${d >= 0 ? '+' : ''}${d.toStringAsFixed(1)}$suffix';
  }

  Color _diffColor(Map<String, dynamic> a, Map<String, dynamic> b, String key, {bool reverse = false}) {
    final va = double.tryParse('${a[key]}');
    final vb = double.tryParse('${b[key]}');
    if (va == null || vb == null) return AppTheme.grisTexte;
    final d = va - vb;
    final bon = reverse ? d > 0 : d < 0;
    return d == 0 ? AppTheme.grisTexte : (bon ? Colors.green : Colors.red);
  }

  @override
  Widget build(BuildContext context) {
    final dernier = bilans.first;
    final premier = bilans.last;

    // Calculer nb semaines approximatif
    final d1 = DateTime.tryParse(premier['date'] ?? '');
    final d2 = DateTime.tryParse(dernier['date'] ?? '');
    String duree = '';
    if (d1 != null && d2 != null) {
    ...
    final resultats = [
      ...
      {'label': 'Masse musculaire', 'key': 'mms', 'suffix': ' kg', 'reverse': true},
    ];
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('RÉSULTATS $duree',
              style: const TextStyle(color: AppTheme.blanc, fontWeight: FontWeight.w800,
        ...
        ...resultats.map((r) {
          final diff = _diff(dernier, premier, r['key'] as String,
          ...
          return Padding(
            ...
            child: Row(children: [
              ...
              Text(diff, style: const TextStyle(color: AppTheme.blanc,
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            const Expanded(flex: 3, child: Text('Indicateur',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppTheme.grisTexte))),
            const Expanded(flex: 2, child: Text('Valeur actuelle',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppTheme.grisTexte), textAlign: TextAlign.center)),
            Expanded(flex: 2, child: Text('Objectif cible',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.green.shade700), textAlign: TextAlign.center)),
            const Expanded(flex: 2, child: Text('À perdre/gagner',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppTheme.grisTexte), textAlign: TextAlign.center)),
          ]),
        ),
        ...
        ...items.where((item) => bilan[item['key']] != null).map((item) {
          ...
          String diff = '—';
          Color diffColor = AppTheme.grisTexte;
          ...
          return Column(children: [
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
              child: Row(children: [
                Expanded(flex: 3, child: Text(item['label'] as String,
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Row(children: [
        ...
        SizedBox(width: 6),
        Text('SYNTHÈSE & ANALYSE COACH', style: TextStyle(fontSize: 12,
            fontWeight: FontWeight.w800, color: AppTheme.orange, letterSpacing: 0.5)),
      ]),
...
class _FormBilanInBodyState extends State<_FormBilanInBody> with SingleTickerProviderStateMixin {
  ...
  final _gvCtrl          = TextEditingController();
        ...
        Expanded(
          child: TabBarView(controller: _tabCtrl, children: [
            ...
            SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
              ...
              const SizedBox(height: 8),
              _row2(_f(_poidsCtrl, 'Poids (kg)'), _f(_mmsCtrl, 'Masse musculaire (kg)')),
              const SizedBox(height: 8),
              _row2(_f(_mmsPctCtrl, 'Masse musculaire (%)'), _f(_masseMaigreCtrl, 'Masse maigre (kg)')),
              const SizedBox(height: 8),
              _row2(_f(_masseOssCtrl, 'Masse osseuse (kg)'), _f(_mgKgCtrl, 'Graisse corporelle (kg)')),
              const SizedBox(height: 8),
            ...
            SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
              ...
              const SizedBox(height: 8),
              _row2(_f(_hydraCtrl, "Taux d'hydratation (%)"), _f(_gvCtrl, 'Graisse viscérale (niveau)')),
              const SizedBox(height: 8),
              _row2(_f(_scoreCtrl, 'Score InBody (/100)', decimal: false), _f(_metaCtrl, 'Métabolisme (kcal)', decimal: false)),
              const SizedBox(height: 8),
